...
function mousePressed(){
   ...
   else if(gameState === 2 && leftyMode === true && mouseIsPressed && mouseX > 605 && mouseX < 695 && mouseY > 525 && mouseY < 615 ){ //left in lefty
    ship.setDir(-1);
   }

   else if (gameState === 2 && leftyMode === true && mouseIsPressed && mouseX > 730 && mouseX < 820 && mouseY > 525 && mouseY < 615){ //right in lefty
    ship.setDir(1);

  }

  else if (gameState === 2 && leftyMode === false && mouseIsPressed && mouseX > 240 && mouseX < 330 && mouseY > 525 && mouseY < 615){ //left in righty
        ship.setDir(-1);
  }
  
  else if (gameState === 2 && leftyMode === false && mouseIsPressed && mouseX > 360 && mouseX < 450 && mouseY > 525 && mouseY < 615){ //right in righty 
        ship.setDir(1);
  }
...
function startGame(){
  ...
  if (leftyMode === true){ // if its in lefty mode draw and assign functionality to left handed controls
   image(leftButton, 650, 570, 80, 80);
   image(rightButton, 775, 570, 80, 80);
   image(fireButton, 360, 570, 80, 80);
  }

  else if (leftyMode === false){ // if its not in lefty mode draw and assign to right handed
   image(leftButton, 285, 570, 80, 80);
   image(rightButton, 405, 570, 80, 80);
   image(fireButton, 730, 570, 80, 80);
  }
  ...
  image(sbLogo, 550, 33, 150, 40);

  image(speakerOff, 710, 30, 40, 40);
  image(speakerOn, 760, 30, 40, 40);
  image(pauseButton, 810, 30, 40, 40); 
  image(leftyButton, 935, 30, 48, 48);
  image(rightyButton, 983, 30, 46, 48);
 ...
 // }
  ...
  if (mouseIsPressed && mouseX > 911 && mouseX < 959 && mouseY > 10 && mouseY < 50){
     leftyMode = true;
    // 
  }

  if (mouseIsPressed && mouseX > 960 && mouseX < 1007 && mouseY > 10 && mouseY < 50){
    // right mode
    leftyMode = false;
  }
...
function keyPressed() {
  ...
  else if (keyCode === 85 || keyCode === 73 || keyCode === 77 || keyCode === 84 || keyCode === 89 || keyCode === 82 || keyCode === 72 || keyCode === 70 || keyCode === 78 || keyCode === 74 || keyCode === 80 || keyCode === 75) { //if keycode equals spacebar 
    var laser = new Laser(ship.x, 510); //create new Laser obj at the current x coord (from the center) of ship and bottom of screen (?)
    lasers.push(laser); //pushes another drop into current lasers array
    laserSound.play();
  }
