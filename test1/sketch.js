...
var gameState = 0;
var couponArray = ["coupon0", "coupon1", "coupon2", "coupon3", "coupon4", "coupon5", "coupon6"];
var coupon;
var couponCounter = 0;
...
function preload(){
  ...
  levelUp = loadSound("levelUp.wav");
  introVid = createVideo("introVid.mp4");
...
function startGame(){
  ...
  text(counter, 900, 600);
  text(mouseX + "," + mouseY, width/6, height-90);
  fill(100,255,100);
  text("Level: ", 510, height -30);
  fill(255);
  text(level, 600, height -30);
  text(score, width-200, height-30);
  fill(100,255,100);
  text("Score:", width-300, height-30);
...
if (score > 10){
      fill(255);
      textSize(20);
      text("HI-SCORE " + coupon, width/2, height/2);
    }

if (gameState === 6){
  for (var i = couponArray.length-1; i >= 0; i--) { //
...
if (edge1){
  ...
  for (var i = 0; i < greenAliens.length; i++) { //function that actually shows and moves the alien array
    ...
    if (greenAliens[i].x > width - 60 || greenAliens[i].x < +60) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
      edge = true; //then edge is there and becomes true
    }
    if (greenAliens[i].y > 300) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
      greenAliens[i].stoned(); //then edge is there and becomes true
    }
    if (greenAliens[i].y > 480) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
  ...
  for (var i = 0; i < pinkAliens.length; i++) { //function that actually shows and moves the alien array
    ...
    if (pinkAliens[i].x > width - 60 || pinkAliens[i].x < +60) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
      edge = true; //then edge is there and becomes true
    }
    if (pinkAliens[i].y > 300) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
      pinkAliens[i].stoned(); //then edge is there and becomes true
    }
    if (pinkAliens[i].y > 480){
      gameState = 6;
    }
  }
  ...
  for (var i = 0; i < blueAliens.length; i++) { //function that actually shows and moves the alien array
    ...
    if (blueAliens[i].x > width - 60 || blueAliens[i].x < +60) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
      edge = true; //then edge is there and becomes true
    }
    if (blueAliens[i].y > 300) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
      blueAliens[i].stoned(); //then edge is there and becomes true
    }
    if (blueAliens[i].y > 480){
      gameState = 6;
    }
  }
  ...
  for (var i = 0; i < yellowAliens.length; i++) { //function that actually shows and moves the alien array
    ...
    if (yellowAliens[i].x > width - 60 || yellowAliens[i].x < +60) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
      edge = true; //then edge is there and becomes true
    }
    if (yellowAliens[i].y > 300) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
      yellowAliens[i].stoned(); //then edge is there and becomes true
    }

    if (yellowAliens[i].y > 480){
      gameState = 6;
    }
  }

  if (chaser.x > width - 60 || chaser.x < + 60) { //if alienarray OBJ x value is greater than width && alienarray x pos is less than 0
      edge = true; //then edge is there and becomes true
    }
...
function levelIncrease() {
 ...
 //  playing = false;
  // gameState = 2;
  //}
  //leveled up same break as fire shiphit
}
...
function keyPressed() {
  ...
  if (keyCode === RIGHT_ARROW) { 
   ...
   yellowAliens.length = 0;
    ...
    var laser = new Laser(ship.x, height-150); //create new Laser obj at the current x coord (from the center) of ship and bottom of screen (?)
...
function resetSketch(){
  ...
  for (var i = 0; i < 10; i++) {  //while i is less than 10 in alien array iterate through array
    greenAliens[i] = new GreenAlien(i*60+100, 30, xdir); //for each alien in array, create a new alien object at i and 60 starting
  }
  for (var i = 0; i < 10; i++) {  //while i is less than 10 in alien array iterate through array
    pinkAliens[i] = new PinkAlien(i*60+100, 80, xdir); //for each alien in array, create a new alien object at i and 60 starting

  }
  for (var i = 0; i < 10; i++) {  //while i is less than 10 in alien array iterate through array
    blueAliens[i] = new BlueAlien(i*60+100, 130, xdir); //for each alien in array, create a new alien object at i and 60 starting

  }
  for (var i = 0; i < 10; i++) {  //while i is less than 10 in alien array iterate through array
    yellowAliens[i] = new YellowAlien(i*60+100, 180, xdir); //for each alien in array, create a new alien object at i and 60 starting
...
function gameOver() { //the only reason x,y are the only parameters is because they are the only perameters that will change throughout us
    ...
    textSize(25);
    fill(255);
    text("Game Over", width/2 -35, 300);
    fill(0,255,0);
    text("Final Score: ", 520, 370);
    fill(255);
    text(score, 675, 370);
    text("Click to Play Again", 500, 440);
   if (mouseIsPressed){
    gameState = 8;
   }
}
...
function EnemyLaser(x, y) {
  ...
  this.r = 3; //initializing object radius to 
  ...
  if (d3 < 20 ) { //if the distance between the laser and alien is less than the radius of laser + the radius of the alien
      return true; //return true value (to delete alien)
    } else { // if else
      return false; //stay false (so alien remains there and nothing happens to it)
    }
...
function GreenAlien(x, y, xdir) { //the only reason x,y are the only parameters is because they are the only perameters that will change throughout use
  this.x = x; //intializing x pos of alien obj
  this.y = y; // intializing y pos of alien object
  this.r = 30; //declaring the radius of the ellipse object
  this.toDelete = false;
  this.xdir = xdir; //delcaring speed as xdir, just 1
...
function Ship() {
  ...
  this.show = function() { //this function will actually show the object
    imageMode(CENTER);
    image(this.img, this.x, this.y, 75, 75); // the x coordinates is the current x value, -20 from the height (bottom) and 20px by 60px
  }
  ...
  /*this.evaporate = function() { //function that deletes current laser when it hits an alien //toDelete is a built in object method(?), deletes the current drop by setting boolean to true (at false in initial state)
    image(this.img2, ship.x, ship.y, 200, 200);
    fire.play();
    fire.loop();
  }*/
...
function NewLife() {
  ...
  this.show = function(lives) { //this function will actually show the object
    textSize(25);
    fill(255);
    text(this.lives, 300, height-30);
    fill(100,255,100);
    text("Lives:", 200, height-30);
    image(this.img, 170, height-40, 30, 30);
  }
...
function Laser(x, y) {
  ...
  if (d < 15) { //if the distance between the laser and alien is less than the radius of laser + the radius of the alien
    image(this.greenPop, this.x, this.y, 30, 30);
      return true; //return true value (to delete alien)
    } else { // if else
      return false; //stay false (so alien remains there and nothing happens to it)
    }
   ...
   if (d1 < 15) { //if the distance between the laser and alien is less than the radius of laser + the radius of the alien
      image(this.pinkPop, this.x, this.y, 30, 30);
      return true; //return true value (to delete alien)
    } else { // if else
      return false; //stay false (so alien remains there and nothing happens to it)
    }
  ...
  if (d2 < 15) { //if the distance between the laser and alien is less than the radius of laser + the radius of the alien
     image(this.bluePop, this.x, this.y, 30, 30);
      return true; //return true value (to delete alien)
    } else { // if else
      return false; //stay false (so alien remains there and nothing happens to it)
    }
  ...
  if (d3 < 15) { //if the distance between the laser and alien is less than the radius of laser + the radius of the alien
      image(this.yellowPop, this.x, this.y, 30, 30);
      return true; //return true value (to delete alien)
    } else { // if else
      return false; //stay false (so alien remains there and nothing happens to it)
    }
...
function BonusShooter(x, y, xdirCop) { //the only reason x,y are the only parameters is because they are the only perameters that will change throughout use
  this.x = x; //intializing x pos of alien obj
  this.y = y; // intializing y pos of alien object
  this.r = 60; //declaring the radius of the ellipse object
  this.toDelete = false;
  ...
  this.show = function() { //method to display the aliens
    //if(frameCount % 3 == 0) {
    imageMode(CENTER);
    image(this.img[(currentCopFrame) % numCopFrames], this.x, this.y, this.r, 38);
    //}
  }
...
function Base (x,y, baseRadius){
 ...
 this.show = function (){
  imageMode(CENTER);
  image(this.img, this.x, this.y, this.baseRadius, this.baseRadius);
}
