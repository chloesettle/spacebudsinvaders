...
function preload(){
  ...
  levelUp = loadSound("levelUp.wav");
  introVid = createVideo("introVid4.mp4");
...
function setup() {
  ...
  xdir = 0.6;
  xdirCop = 1;
  highScoreColor1 = color(255,0,0);
  highScoreColor2 = color(255,0,0);
...
function startGame(){
  ...
  if (leftyMode === true){ // if its in lefty mode draw and assign functionality to left handed controls
   image(leftButton, 700, 575, 55, 55);
   image(rightButton, 760, 575, 55, 55);
   image(fireButton, 360, 575, 55, 55);

        if (mouseIsPressed && mouseX > 670 && mouseX < 730 && mouseY > 550 && mouseY < 600){
      ship.setDir(-1);
        }
  
        if (mouseIsPressed && mouseX > 760 && mouseX < 820 && mouseY > 550 && mouseY < 600){ //this may need to be an else if
          ship.setDir(1);
        }

        if (mouseIsPressed && mouseX > 330 && mouseX < 390 && mouseY > 550 && mouseY < 600){
   ...
   image(leftButton, 330, 575, 55, 55);
   image(rightButton, 390, 575, 55, 55);
   image(fireButton, 750, 575, 55, 55);

       if (mouseIsPressed && mouseX > 300 && mouseX < 360 && mouseY > 550 && mouseY < 600){
        ship.setDir(-1);
      }
  
      if (mouseIsPressed && mouseX > 360 && mouseX < 416 && mouseY > 550 && mouseY < 600){ //this may need to be an else if
        ship.setDir(1);
      }

      if (mouseIsPressed && mouseX > 720 && mouseX < 775 && mouseY > 550 && mouseY < 600){
        var laser = new Laser(ship.x, 510); //create new Laser obj at the current x coord (from the center) of ship and bottom of screen (?)
        lasers.push(laser); //pushes another drop into current lasers array
        laserSound.play();
      }
  ...
  text("Target Score: ", 250, 40);
  fill(highScoreColor2);
  text("Target Score: ", 480, 40);
  fill(255);
  text(highScore1 + " pts", 370, 40);
  text(highScore2 + " pts", 600, 40);
...
if (score > 400000 && score < 419999 && frameCount % 8 == 0){
  highScoreColor1 = green;
}

if (score > 400000 && score < 419999 && frameCount % 16 == 0){
  highScoreColor1 = red;

}

if (score > 950000 && score < 999999 && frameCount % 8 == 0){
  highScoreColor2 = green;

}

if (score > 950000 && score < 999999 && frameCount % 16 == 0){
  highScoreColor2 = red;
...
function gameOver() { //the only reason x,y are the only parameters is because they are the only perameters that will change throughout us
    ...
    text("AAA                  4280", 630, 450);
    text("CHA                   4130", 630, 480);
...
function submitPage(){
    ...
    fill(255);
    text("Submitted!", 490, 250);
    text("Hit ENTER to Play Again", 445, 280);
    nameInput.hide();
    emailInput.hide();
    //button.hide();
